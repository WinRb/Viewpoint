      ...
      class CalendarItem < OpenStruct
        ...
        def to_ews_item
          ...
          PARAMETERS.each do |key|
            ...
            # Convert attributes
            item_parameters[key] = case key
                                   when :start, :end
                                     { text: value.respond_to?(:iso8601) ? value.iso8601 : value }
                                   when :body
                                     { body_type: body_type || 'Text', text: value.to_s }
                                   else
                                     value
                                   end
          end
